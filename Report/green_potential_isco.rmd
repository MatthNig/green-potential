...
# make the table
renderDT(
        datatable(isco_list,  extensions = 'Scroller',
...
renderDT(
        datatable(sec3_dat,  extensions = 'Scroller',
                  class = "display nowrap compact", # style
                  filter = "top", # location of column filters
                  rownames= FALSE,
...
renderPlotly({
        ...
        plot2_data <- subset(sec4_dat,
               ...
               ) %>% mutate(Industry = paste(NOGAS_NAMES, "\nShare of jobs with green potential:", round(green_emp_share, 2), "\nShortage of jobs with green potential:", round(WeightedShortageGreen, 2), "\nTradeability:", round(Handelbarkeit, 2), "\nGreenhouse gas emissions per value added (in logs)", round(log(GHG_per_ValueAdded), 2)))

        # make the plot
        ggplotly(
                ggplot(data = plot2_data, aes(x = green_emp_share, y= WeightedShortageGreen, label = Industry, color = log(GHG_per_ValueAdded), size = Handelbarkeit))+
                geom_point(alpha = 0.7)+
                ...
                ylab(paste("Shortage of jobs with green potential \n (Region: ", 
                           paste0(input$REGIONEN, collapse = " & "),
                           ", green potential of at least ", input$cutOff,")",sep = ""))+
                ylim(min(plot2_data$WeightedShortageGreen), max(plot2_data$WeightedShortageGreen))+
                xlim(min(plot2_data$green_emp_share), max(plot2_data$green_emp_share))+
                guides(color = FALSE)+
                scale_color_viridis(option="viridis", begin = 0.7, end = 0.4, discrete = FALSE, name = "None")+
                theme(axis.title = element_text(face="bold",size = 10),
                      panel.background = element_blank(),
